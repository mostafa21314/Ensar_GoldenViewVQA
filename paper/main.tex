...
We therefore reuse the perception front-end of GeoDrive
\citep{gaafar2025geodrive}, our prior cross-city driving system, and convert
its structured output into text, extended so that each object carries the set
of cameras it was seen in. The reasoning is that if every detected object is
attributed to the specific cameras it appears in, then view selection reduces
to grounding the question to an object and reading off its view-set. We test
that reduction directly, and then test what raw pixels add on top of it.
...
\paragraph{No tracking.} GeoDrive applies BoT-SORT within-camera tracking
before cross-camera association. We removed it. The task input is a single
timestamp, so temporal association yields nothing here; measured on the
GeoDrive artefacts it provided no false-positive suppression (untracked boxes
are retained with $\texttt{track\_id}=-1$), no interpolation, and non-zero
velocity on only $1$--$3\%$ of agents. It also forced sequential whole-scene
processing: serialising our 73 frames through the tracking pipeline would have
required roughly 15{,}800 image passes instead of 438. Cross-camera identity is
not tracking; it is the per-frame association step below, which we keep.
...
\paragraph{Model and compute.} Both systems use Claude Opus~5 with extended
thinking, held fixed across systems so that the System~2 versus System~1
contrast measures the contribution of pixels rather than a change of model. No
model is trained or fine-tuned by us; the only learned weights are the
inherited detector checkpoint. Detection ran on a single consumer GPU in under
two minutes for all 438 images. The 114 records were processed in shards of
9--11 records to bound context per call.
...
\midrule
\multicolumn{5}{l}{\textit{submitted systems}} \\
System 1 (text)   & 0.709 & 0.382 & \textbf{0.945} & 0.709 \\
System 2 (+pixels)& \textbf{0.873} & \textbf{0.679} & 0.927 & \textbf{0.855} \\
\bottomrule
\end{tabular}
...
\paragraph{Official test results.} \textbf{TODO: insert the official
\texttt{test} numbers reported by the GroundLM 2026 evaluator Space for the
submitted run, and state which system produced them.} We deliberately do not
report locally computed test numbers here: test labels are withheld, so no
local figure would be meaningful.
...
without tracking, the front-end yields $29.3$ raw detections per frame and
...
\paragraph{Development versus test.} We expect test performance below
Table~\ref{tab:dev}. Test leans more on \textit{intent\_prediction}
($20/59$ versus $13/55$), which was System~2's weakest group on development
(view $0.692$, versus $0.926$ on causality). Furthermore, System~2's predicted
view distribution on test is more front-heavy than on development
($47/59$ versus $41/55$), which is the direction that costs macro accuracy.
\textsc{none\_of\_the\_above} was again never predicted, so any such test
...
We report a single run per system rather than a distribution over runs, so
sampling variability in the model's own outputs is not quantified. We did not
implement the information-gain formulation we had designed for abstention, so
\textsc{none\_of\_the\_above} remains unreachable in both submitted systems.
...
the motion cues our single-frame design discards by construction. We did not
pursue this because it raises a question about what counts as permissible
input, which we would want the organizers to settle first.
...
Our system emits claims about which camera provides evidence for a driving
judgement. Such claims could invite unwarranted confidence if deployed in a
safety-critical setting. The failure modes in Section~\ref{sec:analysis} are
directly relevant: a camera can be serialised as empty when it is not, and an
object can be attributed to the wrong view. A system that names its evidence
source is not thereby a system whose evidence source is correct, and it should
not be treated as a safety argument without independent verification.
